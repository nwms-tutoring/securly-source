...
!(function () {
    ...
    (globalThis.overlay = {
        ...
        clearOverlay: function () {
            ...
            debuglog.log(`overlay clear:${document.location.toString()} ${t}`),
                (document.documentElement.style.contentVisibility = "");
            ...
            null !== o && o.remove(),
                null !== e && e.remove(),
                setTimeout(() => {
                    if (overlay.overlayId !== t)
                        return void debuglog.log(
                            `overlay clear check skipped: current: ${overlay.overlayId} check: ${t}`
                        );
                    document.documentElement.style.contentVisibility = "";
                    let o = document.getElementById("securlyOverlay"),
                        e = document.getElementById("soverlay-style");
        ...
        filterCheck: function (t) {
            ...
            chrome.runtime.sendMessage(
                { type: "filter-check", url: o, overlayCheckFailures: overlay.overlayCheckFailures },
                function (t) {
        ...
        overlayCheck: function (t) {
            if ((debuglog.log(`overlay check:${document.location.toString()}`), overlay.overlayId !== t))
                return void debuglog.log(`overlay check skipped: current: ${overlay.overlayId} check: ${t}`);
            ...
            (overlay.overlayCheckFailures += 1),
                !o && e
                    ? debuglog.log(`overlay check failed: contenthidden ${t} failures: ${overlay.overlayCheckFailures}`)
                    : debuglog.log(`overlay check failed: ${t} failures: ${overlay.overlayCheckFailures}`),
                overlay.filterCheck(),
                setTimeout(() => {
        ...
        onMessage: function (t, o, e) {
            if (t && "filter-clear" === t.type) {
                ...
                if (t.url !== overlay._getLocation())
                    return (
                        debuglog.log(`overlay clear skipped:${document.location.toString()} ${t.url}`),
                        debuglog.dir(t),
                        e({ type: "filter-clear-ack", priorCleared: o, urlMismatch: !0 }),
                        !1
                    );
                overlay.clearOverlay(),
                    debuglog.log(`overlay cleared:${document.location.toString()}`),
                    debuglog.dir(t),
                    e({ type: "filter-clear-ack", priorCleared: o }),
                    (overlay.cleared = !0);
            }
        },
        ...
        onNavigate: function (o) {
            ...
            if (e.hostname.endsWith("youtube.com") || e.hostname.endsWith("bing.com") || l) {
                ...
                if (e.toString() === overlay._getLocation() && "replace" === o.navigationType)
                    return void debuglog.log(`navigation skipped:${e.toString()} ${o.navigationType}`);
                debuglog.log(`navigating:${e.toString()} ${o.navigationType}`),
                    (document.documentElement.style.contentVisibility = "hidden"),
                    overlay.setOverlay(e),
                    overlay.filterCheck(e.toString()),
                    debuglog.log(`overlay reloaded:${e.toString()}`);
            }
        },
        ...
        checkSpecialHandling: function () {
            const t = new URL(overlay._getLocation());
            t.hostname.endsWith("bing.com") &&
                (debuglog.log(`special handling for bing:${t.toString()}`),
        ...
        init: function () {
            globalThis.navigation &&
                ...
                (document.documentElement.style.contentVisibility = "hidden"),
                document.addEventListener("DOMContentLoaded", overlay._domcontentloaded),
                chrome.runtime.onMessage.addListener(function (t, o, e) {
                    return overlay.onMessage(t, o, e);
                }),
                overlay.filterCheck(),
                overlay.checkSpecialHandling(),
                debuglog.log(`overlay loaded:${document.location.toString()}`);
        },
        ...
        uninit_fortests: function () {
            document.removeEventListener("DOMContentLoaded", overlay._domcontentloaded),
                document.removeEventListener("beforeunload", overlay._beforeunload),
                document.removeEventListener("submit", overlay.bingOnSubmit),
                document.removeEventListener("keydown", overlay.bingOnKeydown),
                globalThis.navigation && navigation.removeEventListener("navigate", overlay.onNavigate),
                (overlay.clearing = !1),
                (overlay.cleared = !1),
                (overlay.overlayId = 0),
                (overlay.overlayCheckFailures = 0);
