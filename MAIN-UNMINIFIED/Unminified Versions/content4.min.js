...
!(function () {
    ...
    var o,
        d,
        r = '<div class="well-path-widget-link-row">{links}</div>',
        ...
        w = function (e) {
            if (
                ...
                -1 != e.target.parentElement.className.indexOf("well-path-widget-link-wrap")
            ) {
                const n = e.target.dataset.href;
                n.startsWith("mailto:") ? (window.location.href = n) : window.open(n),
            ...
            "img" == e.target.nodeName.toLowerCase() &&
                -1 != e.target.parentElement.className.indexOf("well-path-widget-close") &&
                (chrome.runtime.sendMessage({ source: "well-path-widget", action: "closed" }), c()),
                (function (e) {
                    let t = void 0;
                    switch (!0) {
                        ...
                        case "a" === e.target.nodeName.toLowerCase() && "need-help-text-button" === e.target.className:
                            ...
                            (t = document.createElement("div")).setAttribute("class", "need-help-wrapper"),
                                (t.innerHTML = d);
                        ...
                        n &&
                            e.addEventListener("input", () => {
                                e.value.trim().length > 0
                                    ? (n.className = "submit-button")
                                    : (n.className = "disabled-button");
                            });
                    }
                })();
        };
    -1 == window.location.href.indexOf("securly.com") &&
        (function () {
            chrome.runtime.onMessage.addListener((l, a) => {
                        ...
                        case "display":
                            ...
                            !(function (l) {
                                ...
                                for (i = 0; i < a.links.length; i++) {
                                    ...
                                    (c += p.replace("{href}", g).replace("{label}", s(h.label))),
                                        i % 2 == 1 && ((widgetLinks += r.replace("{links}", c)), (c = ""));
                                }
                                c && (widgetLinks += r.replace("{links}", c)),
                                    (widgetHtml = t
                                        .replace("{close_btn_src}", chrome.runtime.getURL("icon-16-px-close.svg"))
                                        .replace("{header}", s(a.header))
                                        .replace("{body}", s(a.body))
                                        .replace("{links}", widgetLinks)),
                                    0 === l.showNeedHelp && (widgetHtml = widgetHtml.replace(n, "")),
                                    ((o = document.createElement("div")).className = "well-path-widget-wrapper"),
                                    (o.innerHTML = widgetHtml),
                ...
                chrome.runtime.sendMessage({ source: "well-path-widget", action: "ready" });
            var l = document.createElement("link");
            (l.rel = "stylesheet"),
                (l.href = chrome.runtime.getURL("fonts/Metropolis.css")),
